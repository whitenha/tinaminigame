...
export default function WheelEditor({ items, onChange }) {
  ...
  const [quizState, setQuizState] = useState(null); // { entry, options[], selected, correct }
  ...
  const entries = items.filter(it => (it.term || '').trim() !== '');

  // ── Sync textarea value from items ─────────────────────────
  const textValue = items.map(it => it.term || '').join('\n');
  ...
  // ── Add image as entry ─────────────────────────────────────
  const handleAddImageAsEntry = useCallback((url) => {
    ...
    setTimeout(() => {
      ...
      if (hasMCQ(won)) {
        ...
        if (wheelConfig.removeWinner) {
          const newItems = items.filter((_, idx) => {
  ...
  const handleQuizAnswer = useCallback((optionIdx) => {
    ...
    setTimeout(() => {
      if (option.correct && wheelConfig.removeWinner) {
        const newItems = items.filter(it => it !== quizState.entry);
        if (newItems.length > 0) onChange(newItems);
      }
      setWinner(null);
      setQuizState(null);
            ...
            <div className={styles.winnerCard} onClick={e => e.stopPropagation()}>
              ...
              <div className={styles.winnerActions}>
                ...
                <button className={styles.winnerRemove} onClick={() => {
                  const newItems = items.filter(it => it !== winner);
                  if (newItems.length > 0) onChange(newItems);
                  setWinner(null);
                }}>
                  Xóa khỏi vòng quay
                ...
                {items.map((item, idx) => (
                  <div key={idx} className={styles.advRow}>
                    ...
                    <input 
                      ...
                      onChange={e => {
                        const newItems = [...items];
                        newItems[idx] = { ...newItems[idx], wrong1: e.target.value };
                        onChange(newItems);
                      }}
                    />
                    <input 
                      ...
                      onChange={e => {
                        const newItems = [...items];
                        newItems[idx] = { ...newItems[idx], wrong2: e.target.value };
                        onChange(newItems);
                      }}
                    />
                    <input 
                      ...
                      onChange={e => {
                        const newItems = [...items];
                        newItems[idx] = { ...newItems[idx], wrong3: e.target.value };
                        onChange(newItems);
                      }}
                    />
                  </div>
            ...
            <div className={styles.advPanel}>
              <label className={styles.advOption}>
                <input
                  ...
                  onChange={e => setWheelConfig(prev => ({...prev, removeWinner: e.target.checked}))}
                />
              ...
              <div className={styles.advOption}>
                ...
                <select
                  ...
                  onChange={e => setWheelConfig(prev => ({...prev, spinDuration: Number(e.target.value)}))}
            ...
            <div className={styles.advPanel}>
              <div className={styles.advOption}>
                ...
                <select
                  ...
                  onChange={e => {
                    setWheelConfig(prev => ({...prev, soundType: e.target.value}));
                    // Preview the sound
                    try {
                      const ctx = getAudioCtx();
                      playTick(ctx, e.target.value, wheelConfig.soundVolume ?? 0.5);
                    } catch(e) {}
                  }}
                  className={styles.advSelect}
                >
              ...
              <div className={styles.advOption}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flex: 1 }}>
                  <input
                    ...
                    onChange={e => setWheelConfig(prev => ({...prev, soundVolume: Number(e.target.value)}))}
