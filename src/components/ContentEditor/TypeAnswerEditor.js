...
export default function TypeAnswerEditor({ item, onChange }) {
  if (!item) return null;
  ...
  const updateAltAnswer = (index, value) => {
    const newOptions = [...(item.options || ['', ''])];
  ...
  const altCount = altAnswers.filter(a => a && a.trim()).length;
          ...
          {altAnswers.map((ans, idx) => (
            <div key={idx} className={styles.altRow}>
              ...
              <input
                ...
                value={ans || ''}
                onChange={(e) => updateAltAnswer(idx, e.target.value)}
                spellCheck={false}
              />
              <button
