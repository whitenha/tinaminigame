...
// ══════════════════════════════════════════════════════════════
export default function MultiplayerMatchupBoard({ items, mp, timeLeft, isSpectatingHost, isShareScreen }: any) {
  ...
  const pairs = useMemo(() => {
    ...
    return pairList
      .filter((it: any) => (it.term || it.question) && (it.definition || ''))
      .map((it: any, idx: any) => ({
        id: idx,
  ...
  const shuffledTerms = useMemo(() => shuffle(pairs.map((p: any) => ({ id: p.id, term: p.term, color: p.color }))), [pairs]);
  // Shuffled definitions (the drop target side)
  const shuffledDefs = useMemo(() => shuffle(pairs.map((p: any) => ({ id: p.id, definition: p.definition }))), [pairs]);

  // State
  const [placements, setPlacements] = useState<any>({});     // { defId: termId }
  const [availableTerms, setAvailableTerms] = useState<any[]>([]); // term ids still in tray
  const [dragItem, setDragItem] = useState<any>(null);        // currently dragging term id
  const [hoveredSlot, setHoveredSlot] = useState<any>(null);  // slot being hovered

  // Track which slot a drag started from (for placed-term drags)
  const dragSourceSlotRef = useRef<any>(null);
  ...
  // ── Drag & Drop (Desktop) ─────────────────────────────────
  const onDragStart = (e: any, termId: any) => {
  ...
  const onPlacedDragStart = (e: any, termId: any, fromDefId: any) => {
    if (isLocked) { e.preventDefault(); return; }
  ...
  const onDragOver = (e: any, defId: any) => {
    if (isLocked) return;
    e.preventDefault();
  ...
  const onDragLeave = (defId: any) => {
    if (isLocked) return;
    if (hoveredSlot === defId) setHoveredSlot(null);
  };

  const onDrop = (e: any, defId: any) => {
    if (isLocked) return;
    e.preventDefault();
  ...
  // ── Touch Drag & Drop ─────────────────────────────────────
  const onTouchStart = (e: any, termId: any) => {
    if (isLocked) return;
  ...
  const placeTerm = (termId: any, defId: any) => {
    sm.click();
  ...
  // ── Remove from slot ──────────────────────────────────────
  const removeFromSlot = (defId: any) => {
    if (isLocked) return;
  ...
  // ── Helpers ────────────────────────────────────────────────
  const getTermById = (id: any) => shuffledTerms.find(t => t.id === id) || pairs.find((p: any) => p.id === id);
