...
import MultiplayerWinOrLoseBoard from './MultiplayerWinOrLoseBoard';
import dynamic from 'next/dynamic';

const WinOrLoseHostBoard = dynamic(() => import('./WinOrLoseHostBoard'), { ssr: false });
...
import gameStyles from '@/components/GameShell/GameShell.module.css';
...
export default function ActiveMultiplayerRoom({ mp, items: rawItems, activity, playerName }: any) {
  ...
  const items = useMemo(() => {
    ...
    if (mp.roomSettings?.shuffledMap && mp.roomSettings.shuffledMap.length === finalItems.length) {
       finalItems = mp.roomSettings.shuffledMap.map((idx: any) => finalItems[idx]);
    }
  ...
  const [eliminatedOptions, setEliminatedOptions] = useState<any[]>([]);
  ...
  const autoFlowRef = useRef<any>(null);
  const answerStatsRef = useRef<any>(null);
  ...
  useEffect(() => {
    ...
    if (item.options) {
      const opts = (Array.isArray(item.options) ? item.options : [])
        .map((opt: any, i: any) => ({ text: typeof opt === 'string' ? opt : (opt?.text || ''), originalIndex: i }))
    ...
    const calculateTimeLeft = () => {
      ...
      // Delay timer for winorlose betting phase (10s)
      if (playerType === 'winorlose') {
        elapsed = Math.max(0, elapsed - 10000);
      }
  ...
  // ── Handle Answer ─────────────────────────────────────────
  const handleAnswer = useCallback(async (originalIndex: any, overridePoints = null) => {
    if (mp.answeredThisQ || selectedAnswer !== null) return;
  ...
  useEffect(() => {
    ...
    // Count active players (exclude spectating hosts like "Host Teacher", must be online)
    const HOST_SPECTATOR_NAMES = ['Host Teacher', 'Giáo viên'];
    const activePlayers = mp.players.filter((p: any) => {
      if (!p.is_online) return false;
      // Exclude spectating hosts (named "Host Teacher" or "Giáo viên")
  ...
  const hasReverseEffect = mp.activeEffects.some((e: any) => e.effectType === 'reverse_controls');
  ...
  if (mp.phase === 'playing' && item) {
    ...
    return (
      <div className={styles.gamePage} style={hasReverseEffect ? { transform: 'scaleX(-1)' } : {}}>
        <GameTopBar 
          counter={playerType === 'groupsort' ? 'Phân Nhóm' : playerType === 'matchup' ? 'Nối Nghĩa' : playerType === 'matchingpairs' ? 'Tìm Đáp Án' : playerType === 'openbox' ? `Hộp ${mp.currentQuestion + 1}` : `${mp.currentQuestion + 1}/${items.length}`} 
          score={mp.myPlayer?.score || 0} 
          streak={mp.myPlayer?.streak || 0} 
          extra={
            activePlayersCount > 0 && (
              <div className={`${gameStyles.topBarPill} ${gameStyles.counterPill} ${answeredCount >= activePlayersCount ? styles.answerProgressComplete : ''}`} style={{ gap: '6px' }}>
                <Icon name="users" size={18} />
                <span>{answeredCount}/{activePlayersCount}</span>
              </div>
            )
          }
        />
        ...
        {false && !mp.isHost && (
          <PowerUpInventory inventory={mp.inventory} onUseItem={(idx: any) => mp.usePowerUp(idx)} activeEffects={mp.activeEffects} itemMultiplier={mp.itemMultiplier} />
        )}
        ...
        {(!isShareScreen || mp.isHost) && !['groupsort', 'matchingpairs', 'matchup', 'hangman', 'minionhangman'].includes(playerType) && (
          <div className={styles.questionSection}>
            <div className={styles.questionCard}>
              {(playerType === 'winorlose' && timeLeft > (item?.extra_data?.time_limit || 20)) ? (
                <h2 className={styles.questionText} style={{ color: '#f1c40f', fontSize: '2rem' }}>
                  <span style={{ display: 'block', fontSize: '1rem', color: '#95a5a6', textTransform: 'uppercase', marginBottom: 8, letterSpacing: 2 }}>Chủ đề</span>
                  {item.extra_data?.topic || 'Bí mật (Cược để xem câu hỏi)'}
                </h2>
              ) : (
                <>
        ...
        <HostLiveFeeds 
          ...
          correctPlayers={
            (playerType === 'unjumble' || playerType === 'spelltheword' || !hasOptions)
              ? mp.correctPlayers?.filter((p: any) => p.correct)
              : mp.correctPlayers
          } 
        ...
        <div className={`${styles.answerArea} ${(['groupsort', 'matchingpairs', 'matchup', 'hangman', 'minionhangman'].includes(playerType)) ? styles.answerAreaFlex : ''}`}>
          {playerType === 'groupsort' ? (
            ...
            <MultiplayerUnjumbleBoard 
              ...
              showFeedback={showFeedback}
            />
          ) : playerType === 'winorlose' ? (
             mp.isHost && isShareScreen ? (
                <WinOrLoseHostBoard item={item} mp={mp} timeLeft={timeLeft} showFeedback={showFeedback} />
             ) : (
                <MultiplayerWinOrLoseBoard
                  item={item}
