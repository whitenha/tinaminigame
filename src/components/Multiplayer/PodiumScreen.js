 ...
 */
export default function PodiumScreen({ leaderboard, myPlayerId, totalQ = 0, onRematch, onExit }) {
  ...
  const myPlayerIndex = useMemo(() => leaderboard.findIndex(p => String(p.id) === String(myPlayerId)), [leaderboard, myPlayerId]);
  ...
  const renderPlace = (player, rank, isVisible) => {
    if (!player) {
      return (
