...
export default function LiveLeaderboard({
  ...
  const top10Ids = leaderboard.slice(0, 10).map((p: any) => p.id);
  const myRealIndex = leaderboard.findIndex((p: any) => p.id === myPlayerId);
  ...
  const renderRow = (player: any, rank: any, i: any, forceMe = false) => {
    // @ts-ignore
  ...
  return (
    <div className={styles.page}>
      ...
      <div className={styles.board}>
        {leaderboard.slice(0, 10).map((player: any, i: any) => renderRow(player, i + 1, i))}
