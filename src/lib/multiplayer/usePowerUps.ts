...
export function usePowerUps() {
  ...
  const usePowerUp = useCallback((slotIndex: any) => {
    ...
    switch (item.effect.type) {
      ...
      case 'restore_streak': {
        if (myPlayer) {
          ...
          // @ts-ignore
          s.setPlayers((prev: any) => prev.map((p: any) => p.id === playerId ? { ...p, streak: item.effect.value } : p));
        }
        break;
      }
      ...
      case 'hide_correct': {
        const otherPlayers = players.filter((p: any) => p.id !== playerId && !p.is_host);
      ...
      case 'mass_damage': {
        const others = players.filter((p: any) => p.id !== playerId && !p.is_host);
        ...
        // @ts-ignore
        s.setPlayers((prev: any) => prev.map((p: any) => {
          if (targets.find((t: any) => t.id === p.id)) return { ...p, score: Math.max(0, (p.score || 0) - item.effect.value) };
          return p;
        }));
        break;
      }
