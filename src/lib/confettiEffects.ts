...
export async function fireStreak(streakCount: number = 3): Promise<void> {
  const c = await getConfetti();
  if (!c) return;
...
export function flashGreen(): void {
...
export function flashRedShake(): void {
  if (typeof document === 'undefined') return;
  // Red vignette
  const el = document.createElement('div');
  el.style.cssText = `
...
export function showTimerDanger(): void {
  if (typeof document === 'undefined') return;
...
export function hideTimerDanger(): void {
  if (typeof document === 'undefined') return;
