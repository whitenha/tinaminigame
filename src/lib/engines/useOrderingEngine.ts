...
export function useOrderingEngine(items: any[], options: Record<string, any> = {}) {
  const {
    musicType = 'calm',
  ...
  // ── Puzzle State ────────────────────────────────────────
  const [pieces, setPieces] = useState<any[]>([]);      // Current arrangement (user moves)
  const [correctOrder, setCorrectOrder] = useState<any[]>([]); // Answer key
  const [selectedPiece, setSelectedPiece] = useState<any>(null);
  const [placedPieces, setPlacedPieces] = useState<any[]>([]); // For slot-based placement
  ...
  const generatePuzzle = useCallback((qIdx: any) => {
  ...
  // ── Swap Two Pieces ─────────────────────────────────────
  const swapPieces = useCallback((fromIdx: any, toIdx: any) => {
  ...
  const selectPiece = useCallback((idx: any) => {
    if (showFeedback) return;
  ...
  const placePiece = useCallback((pieceIdx: any) => {
    if (showFeedback) return;
    emit(GameEvent.CLICK);
  ...
  const removePlaced = useCallback((placedIdx: any) => {
    if (showFeedback) return;
