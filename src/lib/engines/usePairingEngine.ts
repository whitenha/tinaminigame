...
export function usePairingEngine(items: any[], options: Record<string, any> = {}) {
  const {
    musicType = 'calm',
  ...
  const [failedPair, setFailedPair] = useState<any>(null);
  ...
  const flipCard = useCallback((cardId: any) => {
    if (phase !== 'playing') return;
  ...
  const selectTerm = useCallback((termIdx: any) => {
    if (matchedItems.has(termIdx)) return;
    emit(GameEvent.CLICK);
    setSelectedTerm(termIdx);
  }, [matchedItems, emit]);

  const selectDef = useCallback((defIdx: any) => {
  ...
  const isCardFlipped = (cardId: any) => flippedCards.includes(cardId);
  const isCardMatched = (cardId: any) => {
    const card = cards.find(c => c.id === cardId);
    return card ? matchedPairs.has(card.pairId) : false;
  };
  const isCardFailed = (cardId: any) => failedPair?.includes(cardId);
