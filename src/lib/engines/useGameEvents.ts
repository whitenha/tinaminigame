...
import GameEventBus from '@/lib/gameEvents';

export function useGameEvents(musicType: string = 'quiz') {
  ...
  useEffect(() => {
    ...
    const s = (): SoundManager | null => soundRef.current;
    ...
    const wirings = [
      ...
      // ── Music ─────────────────────────────────────────
      bus.on(GameEvent.MUSIC_START, (p) => s()?.startMusic((p?.type as string) || musicType)),
      ...
      bus.on(GameEvent.STREAK_BONUS, (p) => {
        s()?.streakFire();
        s()?.bonus();
        fireStreak((p?.streak as number) || 3);
      }),
