...
export function useActionEngine(items: any[], options: Record<string, any> = {}) {
  const {
    musicType = 'fun',
  ...
  const [showHitEffect, setShowHitEffect] = useState<any>(null); // {x, y, correct}
  ...
  useEffect(() => {
    ...
    const spawn = () => {
      ...
      const usedSlots = s.entities.map((e: any) => e.slot);
      let slot;
      do {
  ...
  const tapEntity = useCallback((entityId: any) => {
    ...
    const entity = stateRef.current.entities.find((e: any) => e.id === entityId);
    if (!entity) return;
