...
import type { GamePhase, ShuffledOption, AnswerRecord } from '@/types';

interface SelectionEngineOptions {
  musicType?: string;
...
interface ContentItemWithOptions {
  question?: string;
  term?: string;
  definition?: string;
  options: string[];
  extra_data?: { time_limit?: number; [key: string]: unknown };
  image_url?: string | null;
  audio_url?: string | null;
}

export function useSelectionEngine(items: ContentItemWithOptions[], options: SelectionEngineOptions = {}) {
  const {
    musicType = 'quiz',
  ...
  // ── Core State ──────────────────────────────────────────
  const [phase, setPhase] = useState<GamePhase>(hasCountdown ? 'countdown' : 'playing');
  ...
  const shuffleOptionsForQuestion = useCallback((qIdx: number): void => {
  ...
  const handleSubmitAnswer = useCallback((selectedOriginalIndex: number): void => {
  ...
  const handleAdvanceToNext = useCallback((): void => {
    const s = stateRef.current;
    if (s.currentQ + 1 < s.items.length) {
      ...
      const opts = s.items[next]?.options
        ?.map((opt: any, i: any) => ({ text: opt, originalIndex: i }))
