...
export function getPowerUp(id: any) {
  // @ts-ignore
 ...
 */
export function addToInventory(inventory: any, newItem: any) {
 ...
 */
export function useFromInventory(inventory: any, slotIndex: any) {
  if (slotIndex < 0 || slotIndex >= inventory.length) {
    return { item: null, inventory };
  }
  const item = inventory[slotIndex];
  const newInventory = inventory.filter((_: any, i: any) => i !== slotIndex);
