...
class SoundManager {
  ...
  volume: number;
  musicVolume: number;
  currentMusic: boolean | number | null;
  private currentMusicHowl: any; // Howl instance
  ...
  private _loadSavedVolumes(): void {
    try {
  ...
  private _resumeCtx(): AudioContext | null {
  ...
  private _playTone(freq: number, duration: number, type: OscillatorType = 'sine', vol: number = 0.3, delay: number = 0): void {
  ...
  private _playNoise(duration: number, vol: number = 0.1, delay: number = 0): void {
    if (!this.enabled || this.volume <= 0) return;
    const ctx = this._resumeCtx();
    if (!ctx) return;
  ...
  countdownBeep(isGo: boolean = false): void {
    if (isGo) {
  ...
  drumroll(durationMs: number = 2000): void {
    if (!this.enabled) return;
