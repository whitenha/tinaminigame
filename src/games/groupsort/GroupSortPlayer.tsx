...
export default function GroupSortPlayer({ items, activity, playerName, gameMode = 'batch' }: any) {
  // 1. GAME STATES
  ...
  const columns = useMemo(() => {
    return items.map((it: any, idx: any) => {
  ...
  const [placements, setPlacements] = useState<any>({}); // { pieceId: colId | null }
  const [placementHistory, setPlacementHistory] = useState([{}]);
  const [selectedPieceId, setSelectedPieceId] = useState<any>(null);
  const [draggedPieceId, setDraggedPieceId] = useState<any>(null);
  const [dragOverColId, setDragOverColId] = useState<any>(null);
  
  // Feedback states
  const [showFeedback, setShowFeedback] = useState(false);
  const [isCorrect, setIsCorrect] = useState(false);
  const [wrongPieceIds, setWrongPieceIds] = useState<any[]>([]); // Array of piece IDs that are wrongly placed
  const [lockedPieceIds, setLockedPieceIds] = useState<any[]>([]); // Array of piece IDs correctly placed and locked
  
  // Instant mode survival states
  const [failedLog, setFailedLog] = useState<any>({}); // { pieceId: count }
  const [destroyedPieceIds, setDestroyedPieceIds] = useState<any[]>([]);
  const [purgedPieceIds, setPurgedPieceIds] = useState<any[]>([]); // Completely removed from DOM LG after animation
  ...
  const evaluateInstantMove = (pieceId: any, colId: any) => {
  ...
  const movePiece = (pieceId: any, colId: any) => {
