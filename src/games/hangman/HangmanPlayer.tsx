...
function RocketSVG({ stage, won, lost }: any) {
...
// ── HANGMAN ENGINE HOOK ──────────────────────────────────────
function useHangmanEngine({ item, timePerSlide, onSlideComplete, isFirstSlide, onGameStart }: any) {
  ...
  const [lastGuessResult, setLastGuessResult] = useState<any>(null); // 'correct' | 'wrong' | null
  ...
  const uniqueWordLetters = [...new Set(word.split('').filter((c: any) => /[A-Z]/.test(c)))];
  ...
  const handleComplete = useCallback((won: any) => {
    setPhase('done');
  ...
  // 4. Guess a letter
  const guessLetter = useCallback((letter: any) => {
    if (phase !== 'playing') return;
...
function HangmanSlide({ item, slideIndex, totalSlides, timePerSlide, onSlideComplete, globalScore, isFirstSlide, onGameStart }: any) {
...
// ── MAIN PLAYER WRAPPER ──────────────────────────────────────
export default function HangmanPlayer({ items, activity, playerName }: any) {
