...
//  SpinWheelPlayer
// ═════════════════════════════════════════════════════════════
export default function SpinWheelPlayer({ items, activity, playerName }: any) {
  ...
  const entries = remainingItems.filter((it: any) => (it.term || '').trim() !== '');
  ...
  // Check MCQ
  const hasMCQ = (entry: any) => entry.definition && entry.extra_data?.wrong1 && entry.extra_data?.wrong2 && entry.extra_data?.wrong3;
  ...
  const handleSpin = useCallback(() => {
    ...
    setTimeout(() => {
      ...
      if (hasMCQ(won)) {
        ...
        if (removeWinner) {
          setRemainingItems((prev: any) => prev.filter((it: any) => it !== won));
        }
      }
    }, dur * 1000 + 100);
  ...
  const handleQuizAnswer = useCallback((optionIdx: any) => {
    ...
    setTimeout(() => {
      if (option.correct && removeWinner) {
        setRemainingItems((prev: any) => prev.filter((it: any) => it !== quizState.entry));
      }
      setWinner(null);
      setQuizState(null);
    }, 2500);
