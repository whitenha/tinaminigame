...
function initParticles(container: any) {
  if (!container) return;
...
// ══════════════════════════════════════════════════════════════
export default function MatchingPairsPlayer({ items: rawItems, activity, playerName, onFinish }: any) {
  ...
  const handleNextRound = (roundScore: any) => {
    setTotalScore(prev => prev + roundScore);
    setCurrentRound(r => r + 1);
  };

  const handleFinish = (roundScore: any) => {
...
function MatchingPairsRound({ items, totalScore, isLastRound, onNextRound, onRestartGame, onFinish, title }: any) {
  ...
  const [feedbackMsg, setFeedbackMsg] = useState<any>(null);
