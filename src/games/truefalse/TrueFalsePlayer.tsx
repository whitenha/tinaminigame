...
export default function TrueFalsePlayer({ items, activity, playerName }: any) {
  ...
  useEffect(() => {
    if (engine.phase === 'playing' && items[engine.currentQ]) {
      ...
      const validOpts = (item.options || []).filter((o: any) => o && o.trim() !== '');
      if (validOpts.length >= 2) {
        const hasTrue = validOpts.some((o: any) => ['đúng', 'true'].includes(o.toLowerCase()));
        const hasFalse = validOpts.some((o: any) => ['sai', 'false'].includes(o.toLowerCase()));
        if (hasTrue && hasFalse) {
