...
// ── Particle Spawner ─────────────────────────────────────────
function spawnDealParticles(container) {
  if (!container) return;
...
//  MAIN COMPONENT
// ══════════════════════════════════════════════════════════════
export default function SpeakingCardsPlayer({ items: rawItems = [], activity, playerName, onFinish }) {
  const cards = useMemo(() => {
    return rawItems
        ...
        <div className={styles.overlay} onClick={() => setShowSettings(false)}>
          <div className={styles.modal} onClick={e => e.stopPropagation()}>
            ...
            <div className={styles.modalBody}>
              ...
              <select className={styles.vSelect} value={enVoice?.name || ''} onChange={e => setEnVoice(voices.find(v => v.name === e.target.value))}>
              ...
              <select className={styles.vSelect} value={viVoice?.name || ''} onChange={e => setViVoice(voices.find(v => v.name === e.target.value))}>
                <option value="">Mặc định</option>
