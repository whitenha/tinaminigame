...
function buildGameData(items: any) {
  // @ts-ignore
  const columns = [];
  // @ts-ignore
  const pieces = [];
  ...
  items.forEach((item: any) => {
    ...
    keywords.forEach((kw: any, ki: any) => {
      pieces.push({
        id: `${colId}_p${ki}_${kw}`,
        text: kw,
        correctColId: colId,
...
export default function CategorizePlayer({ items, activity, playerName, gameMode = 'batch' }: any) {
  // ── Phase management ──
  ...
  const [results, setResults] = useState<any[]>([]); // { pieceId, nestId, correct }
  const [combo, setCombo] = useState(0);
  const [maxCombo, setMaxCombo] = useState(0);

  // ── Physics state (refs for perf) ──
  const birdRef = useRef<any>(null);       // DOM ref for the flying bird
  const canvasRef = useRef<any>(null);     // Particle canvas
  const containerRef = useRef<any>(null);  // Game container
  const nestRefs = useRef<any>({});        // { colId: DOM element }
  ...
  const [feedbackText, setFeedbackText] = useState<any>(null); // { text, type: 'correct'|'wrong', nestId }
  ...
  // ── Collision Handlers ──
  const handleNestHit = useCallback((colId: any) => {
    const piece = queue[0];
    if (!piece) return;
