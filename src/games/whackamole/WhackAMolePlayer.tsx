...
function useWhackSlideEngine({ item, timePerSlide, heartsConfig, onSlideComplete, isFirstSlide, onGameStart }: any) {
  ...
  const [showHitEffect, setShowHitEffect] = useState<any>(null);
  ...
  const tapEntity = useCallback((entityId: any) => {
    if (phase !== 'playing') return;
...
function WhackAMoleSlide({ item, slideIndex, totalSlides, timePerSlide, heartsConfig, onSlideComplete, globalScore, isFirstSlide, onGameStart }: any) {
...
// ── PLAY PAGE WRAPPER ─────────────────────────────────────────
export default function WhackAMolePlayer({ items, activity, playerName }: any) {
  ...
  const openerAudioRef = useRef<any>(null);
  ...
  useEffect(() => {
    ...
    openerAudioRef.current.play().catch((e: any) => console.warn('BGM AutoPlay blocked:', e));
  ...
  const handleGameStart = useCallback(() => {
    if (currentSlideIndex === 0 && bgmAudioRef.current) {
      bgmAudioRef.current.play().catch((e: any) => console.warn('BGM Blocked:', e));
    }
  }, [currentSlideIndex]);

  const handleSlideComplete = (slideStats: any) => {
